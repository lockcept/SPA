...
class StandardOutputHandler(KVWriter, StrWriter):
    def __init__(self, filename_or_textio: Union[str, TextIO]) -> None:
        ...
        if isinstance(filename_or_textio, str):
            ...
            assert hasattr(
                filename_or_textio, "write"
            ), "Expected file or str, got {}".format(filename_or_textio)
...
class Logger(object):
        ...
        for file_name, fmt in output_config.items():
            try:
                self._output_handlers.append(
                    HANDLER[fmt](os.path.join(self._record_dir, file_name))
                )
            except KeyError:
                warnings.warn(
                    "Invalid output type, Valid types: stdout, csv, tensorboard",
                    DeprecationWarning,
                )
    ...
    def log_hyperparameters(self, hyper_param: Dict) -> None:
        json_output_handler = JSONOutputHandler(
            os.path.join(self._record_dir, "hyper_param")
        )
